...
int cortex_m_set_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	LOG_DEBUG("BPID: %" PRIu32 ", Type: %d, Address: 0x%" PRIXX " Length: %d (set=%d)",
		breakpoint->unique_id,
		(int)(breakpoint->type),
		breakpoint->address,
		breakpoint->length,
...
static int cortex_m_read_memory(struct target *target, target_ulong address,
...
static int cortex_m_write_memory(struct target *target, target_ulong address,

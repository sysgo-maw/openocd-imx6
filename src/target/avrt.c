...
static int avr_resume(struct target *target, int current, target_ulong address,
		int handle_breakpoints, int debug_execution);
static int avr_step(struct target *target, int current, target_ulong address,
...
static int avr_step(struct target *target, int current, target_ulong address, int handle_breakpoints)
{
	LOG_DEBUG("%s", __func__);
	return ERROR_OK;
}

...
static int dsp5680xx_resume(struct target *target, int current,
			    target_ulong address, int hb, int d)
{
	if (target->state == TARGET_RUNNING) {
...
static int dsp5680xx_read(struct target *t, target_ulong a, uint32_t size,
 ...
 * @return
 */
static int dsp5680xx_write(struct target *t, target_ulong a, uint32_t s, uint32_t c,
			   const uint8_t *b)
{
...
static int dsp5680xx_write_buffer(struct target *t, target_ulong a, uint32_t size,
				  const uint8_t *b)
{
 ...
 * @return
 */
static int dsp5680xx_read_buffer(struct target *t, target_ulong a, uint32_t size,
				 uint8_t *buf)
{
	check_halt_and_debug(t);
 ...
 * @return
 */
static int dsp5680xx_checksum_memory(struct target *t, target_ulong a, uint32_t s,
				     uint32_t *checksum)
{
	return ERROR_FAIL;
}
...
static int dsp5680xx_step(struct target *target, int current, target_ulong address,

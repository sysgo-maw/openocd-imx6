...
int nds32_virtual_to_physical(struct target *target, target_ulong address, target_ulong *physical)
{
	struct nds32 *nds32 = target_to_nds32(target);
...
int nds32_cache_sync(struct target *target, target_ulong address, uint32_t length)
{
	struct aice_port_s *aice = target_to_aice(target);
	struct nds32 *nds32 = target_to_nds32(target);
...
{
	LOG_DEBUG("current %d address %" PRIXX

...
static int calcaddr_pyhsfromlin(struct target *t, target_ulong addr,
...
int x86_32_common_virt2phys(struct target *t, target_ulong address, target_ulong *physical)
{
	struct x86_32_common *x86_32 = target_to_x86_32(t);
	...
	if (!(cr0 & CR0_PG)) {
		...
		*physical = dsb + address;
		...
		if (calcaddr_pyhsfromlin(t, address, physical) != ERROR_OK) {
			LOG_ERROR("%s failed to calculate physical address from 0x%" PRIXX,
					__func__, address);
			return ERROR_FAIL;
		}
	}
...
int x86_32_common_read_phys_mem(struct target *t, target_ulong phys_address,
			uint32_t size, uint32_t count, uint8_t *buffer)
{
	struct x86_32_common *x86_32 = target_to_x86_32(t);
	int error;
...
int x86_32_common_write_phys_mem(struct target *t, target_ulong phys_address,
			uint32_t size, uint32_t count, const uint8_t *buffer)
{
	struct x86_32_common *x86_32 = target_to_x86_32(t);
	int error = ERROR_OK;
	...
	check_not_halted(t);
	if (!count || !buffer || !phys_address) {
		LOG_ERROR("%s invalid params count=0x%" PRIx32 ", buf=%p, addr=0x%" PRIXX,
				__func__, count, buffer, phys_address);
		return ERROR_COMMAND_ARGUMENT_INVALID;
	}
...
int calcaddr_pyhsfromlin(struct target *t, target_ulong addr, target_ulong *physaddr)
{
...
int x86_32_common_read_memory(struct target *t, target_ulong addr,
			uint32_t size, uint32_t count, uint8_t *buf)
{
	int retval = ERROR_OK;
	struct x86_32_common *x86_32 = target_to_x86_32(t);
	LOG_DEBUG("addr=0x%" PRIXX ", size=%" PRIu32 ", count=0x%" PRIx32 ", buf=%p",
			addr, size, count, buf);
	check_not_halted(t);
	if (!count || !buf || !addr) {
		LOG_ERROR("%s invalid params count=0x%" PRIx32 ", buf=%p, addr=0x%" PRIXX,
	...
	if (x86_32->is_paging_enabled(t)) {
		...
		target_ulong physaddr = 0;
		if (calcaddr_pyhsfromlin(t, addr, &physaddr) != ERROR_OK) {
			LOG_ERROR("%s failed to calculate physical address from 0x%" PRIXX, __func__, addr);
			retval = ERROR_FAIL;
		}
		...
		if (retval == ERROR_OK
			...
			LOG_ERROR("%s failed to read memory from physical address 0x%" PRIXX, __func__, physaddr);
			retval = ERROR_FAIL;
		}
		...
		if (x86_32_common_read_phys_mem(t, addr, size, count, buf) != ERROR_OK) {
			LOG_ERROR("%s failed to read memory from address 0%" PRIXX, __func__, addr);
			retval = ERROR_FAIL;
		}
	}
...
int x86_32_common_write_memory(struct target *t, target_ulong addr,
			uint32_t size, uint32_t count, const uint8_t *buf)
{
	...
	if (x86_32->is_paging_enabled(t)) {
		...
		if (retval == ERROR_OK
			...
			LOG_ERROR("%s failed to write memory to physical address 0x%" PRIXX,
					__func__, physaddr);
			retval = ERROR_FAIL;
		}
		/* restore PG bit if it was cleared prior (regardless of retval) */
		...
		if (x86_32_common_write_phys_mem(t, addr, size, count, buf) != ERROR_OK) {
			LOG_ERROR("%s failed to write memory to address 0x%" PRIXX,
					__func__, addr);
			retval = ERROR_FAIL;
		}
	}
...
{
	LOG_DEBUG("type=%d, addr=0x%" PRIXX, bp->type, bp->address);
	if (check_not_halted(t))
		return ERROR_TARGET_NOT_HALTED;
...
static int unset_hwbp(struct target *t, struct breakpoint *bp)
{
	...
	LOG_USER("%s hardware breakpoint %" PRIu32 " removed from 0x%" PRIXX " (hwreg=%d)",
...
static int set_swbp(struct target *t, struct breakpoint *bp)
{
	...
	if (readback != SW_BP_OPCODE) {
		LOG_ERROR("%s software breakpoint error at 0x%" PRIXX ", check memory",
				__func__, bp->address);
	...
	LOG_USER("%s software breakpoint %" PRIu32 " set at 0x%" PRIXX,
...
static int unset_swbp(struct target *t, struct breakpoint *bp)
{
	...
	if (current_instr == SW_BP_OPCODE) {
		...
		LOG_ERROR("%s software breakpoint remove error at 0x%" PRIXX ", check memory",
				__func__, bp->address);
	...
	LOG_USER("%s software breakpoint %" PRIu32 " removed from 0x%" PRIXX,
			__func__, bp->unique_id, bp->address);
	return ERROR_OK;
}
...
static int set_breakpoint(struct target *t, struct breakpoint *bp)
{
	...
	if (bp->type == BKPT_HARD) {
		error = set_hwbp(t, bp);
		if (error != ERROR_OK) {
			LOG_ERROR("%s error setting hardware breakpoint at 0x%" PRIXX,
					__func__, bp->address);
			return error;
		}
	} else {
		if (x86_32->sw_bpts_supported(t)) {
			error = set_swbp(t, bp);
			if (error != ERROR_OK) {
				LOG_ERROR("%s error setting software breakpoint at 0x%" PRIXX,
						__func__, bp->address);
				return error;
			}
		} else {
...
static int unset_breakpoint(struct target *t, struct breakpoint *bp)
{
	...
	if (bp->type == BKPT_HARD) {
		if (unset_hwbp(t, bp) != ERROR_OK) {
			LOG_ERROR("%s error removing hardware breakpoint at 0x%" PRIXX,
					__func__, bp->address);
			return ERROR_FAIL;
		}
	} else {
		if (unset_swbp(t, bp) != ERROR_OK) {
			LOG_ERROR("%s error removing software breakpoint at 0x%" PRIXX,
					__func__, bp->address);
			return ERROR_FAIL;
		}
	}
...
static int set_watchpoint(struct target *t, struct watchpoint *wp)
{
	...
	struct x86_32_dbg_reg *debug_reg_list = x86_32->hw_break_list;
	int wp_num = 0;
	LOG_DEBUG("type=%d, addr=0x%" PRIXX, wp->rw, wp->address);
	...
	LOG_USER("'%s' watchpoint %d set at 0x%" PRIXX " with length %" PRIu32 " (hwreg=%d)",
...
static int unset_watchpoint(struct target *t, struct watchpoint *wp)
{
	...
	LOG_USER("'%s' watchpoint %d removed from 0x%" PRIXX " with length %" PRIu32 " (hwreg=%d)",
			wp->rw == WPT_READ ? "read" : wp->rw == WPT_WRITE ?
			"write" : wp->rw == WPT_ACCESS ? "access" : "?",
			wp->unique_id, wp->address, wp->length, wp_num);

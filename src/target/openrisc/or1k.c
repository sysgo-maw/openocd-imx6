			       ...
			       struct breakpoint *breakpoint)
{
	...
	LOG_DEBUG("Adding breakpoint: addr 0x%" PRIXX ", len %d, type %d, set: %d, id: %" PRId32,
	...
	int retval = du_core->or1k_jtag_read_memory(&or1k->jtag,
					 ...
					 &data);
	if (retval != ERROR_OK) {
		LOG_ERROR("Error while reading the instruction at 0x%" PRIXX,
			   breakpoint->address);
		return retval;
	}
	...
	if (retval != ERROR_OK) {
		LOG_ERROR("Error while writing OR1K_TRAP_INSTR at 0x%" PRIXX,
			   breakpoint->address);
		return retval;
	}
				  ...
				  struct breakpoint *breakpoint)
{
	...
	LOG_DEBUG("Removing breakpoint: addr 0x%" PRIXX ", len %d, type %d, set: %d, id: %" PRId32,
		  breakpoint->address, breakpoint->length, breakpoint->type,
		  breakpoint->set, breakpoint->unique_id);
	...
	if (retval != ERROR_OK) {
		LOG_ERROR("Error while writing back the instruction at 0x%" PRIXX,
			   breakpoint->address);
		return retval;
	}
...
static int or1k_read_memory(struct target *target, target_ulong address,
		uint32_t size, uint32_t count, uint8_t *buffer)
{
	struct or1k_common *or1k = target_to_or1k(target);
	struct or1k_du *du_core = or1k_to_du(or1k);

	LOG_DEBUG("Read memory at 0x%" PRIXX ", size: %" PRIu32 ", count: 0x%08" PRIx32, address, size, count);
...
static int or1k_write_memory(struct target *target, target_ulong address,
		uint32_t size, uint32_t count, const uint8_t *buffer)
{
	struct or1k_common *or1k = target_to_or1k(target);
	struct or1k_du *du_core = or1k_to_du(or1k);

	LOG_DEBUG("Write memory at 0x%" PRIXX ", size: %" PRIu32 ", count: 0x%08" PRIx32, address, size, count);
...
static int or1k_checksum_memory(struct target *target, target_ulong address,

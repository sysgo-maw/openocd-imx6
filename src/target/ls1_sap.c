...
static int ls1_sap_resume(struct target *target, int current, target_ulong address,
...
static int ls1_sap_step(struct target *target, int current, target_ulong address,
...
static int ls1_sap_read_memory(struct target *target, target_ulong address,
			       uint32_t size, uint32_t count, uint8_t *buffer)
{
	LOG_DEBUG("Reading memory at physical address 0x%" PRIXX
...
static int ls1_sap_write_memory(struct target *target, target_ulong address,
				uint32_t size, uint32_t count,
				const uint8_t *buffer)
{
	LOG_DEBUG("Writing memory at physical address 0x%" PRIXX
		  "; size %" PRId32 "; count %" PRId32, address, size, count);
